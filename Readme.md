...
|-------|-------|----------|
| "No data found" in reports | Timezone date parsing was wrong | Fixed in current version; hard refresh (`Ctrl+F5`) to clear cache |
| Listener stacking (multiple subscriptions) | Old monitor subscription not cleaned up | Now properly unsubscribes when switching sessions |
| Student cache persisting across users | Window cache not cleared on login/logout | Cache now cleared on every auth state change |
| Instructor can see/approve students from other sections | Missing scope check | Now filtered by instructor's section keys |
| QR code not centered | Missing CSS flex rules | CSS enhanced with centering rules |
...
- Attendance analytics dashboard with charts and trends
- Bulk CSV import for students/subjects
- Parent notifications for attendance
- Geolocation verification for off-campus attendance